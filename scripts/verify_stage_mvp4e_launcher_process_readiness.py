...
def main() -> int:
    ...
    ros2_wrapper = read_rel("audit/run_in_ros2_lyrical.ps1")
    ...
    cases.append(case("incremental_log_write_supported", "FileShare]::ReadWrite" in launcher and "Start-Sleep -Milliseconds 200" in launcher))
    cases.append(case("live_log_reader_disposes_handles", "StreamReader" in launcher and ".Dispose()" in launcher))
    cases.append(case("ros2_wrapper_native_stderr_stringified", "System.Management.Automation.ErrorRecord" in ros2_wrapper and "Write-Output ([string]$_.Exception.Message)" in ros2_wrapper))
    cases.append(case("ros2_wrapper_preserves_native_exit_code", "$exitCode = $LASTEXITCODE" in ros2_wrapper and "exit $exitCode" in ros2_wrapper))
